...
class Envelope(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x_pow_p1 = x_pow_p0 * x
        x_pow_p2 = x_pow_p1 * x
        out = 1. / x + a * x_pow_p0 + b * x_pow_p1 + c * x_pow_p2
        return out * C
...
class dist_emb(torch.nn.Module):
    ...
    def forward(self, dist):
        #print(f"Debug dimepp dist : {dist}")
        dist = dist.unsqueeze(-1) / self.cutoff
        #print(f"Debug cutoff : {self.cutoff}")
        #print(f"Debug dimepp envelope(dist) : {self.envelope(dist)}")
        # masking here
